...
void stdout_example()
{
    ...
    spdlog::get("stdout")->info("loggers can be retrieved from a global registry using the spdlog::get(logger_name)");
}
```
...
void binary_example()
{
    ...
    stdout->info("Binary example: {}", spdlog::to_hex(buf));
    stdout->info("Another binary example:{:n}", spdlog::to_hex(std::begin(buf), std::begin(buf) + 10));
    // more examples:
...
// The stdout will show only warnings or errors, while the file will log all.
void multi_sink_example()
{
    ...
    logger.warn("this should appear in both stdout and file");
    logger.info("this message should not appear in the stdout, only in the file");
}
```
...
void err_handler_example()
{
    ...
    spdlog::set_error_handler([](const std::string &msg) { spdlog::get("stdout")->error("*** LOGGER ERROR ***: {}", msg); });
    spdlog::get("stdout")->info("some invalid message to trigger an error {}{}{}{}", 3);
}
